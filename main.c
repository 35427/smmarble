...
float gradeAverage(int player); //calculate average grade of the player
    ...
    switch(type) {
        case SMMNODE_TYPE_LECTURE: // 강의 칸에 도착 
            ...
            if (lec == 1){
               ...
               printf("들은 학점은 %d, 남은 에너지는 %d, 수강한 강의는 %s, 학점은 %s = %f입니다.\n",smmObj_getNodeCredit(boardPtr), cur_player[player].energy,cur_player[player].currentLecture, smmObj_getGradeName(smmObj_getNodeGrade(gradePtr)),gradeAverage(player) );
               // 수강한 강의를 추가
